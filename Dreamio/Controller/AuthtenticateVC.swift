...
class AuthtenticateVC: UIViewController {
    ...
    func authenticate() {
        ...
        if context.canEvaluatePolicy(policy, error: &authError) {
            context.evaluatePolicy(policy, localizedReason: "To unlock Cannaseur.") { [weak self] (success, err) in
    ...
    func showEnterPasscode() {
        ...
        if context.canEvaluatePolicy(policy, error: &errMess) {
            context.evaluatePolicy(policy, localizedReason: "Please authenticate to unlock Cannseur.") { [unowned self] (success, err) in
                DispatchQueue.main.async {
                    if success && err == nil {
                        self.performSegue(withIdentifier: Segues.PasscodeVCToTabbar, sender: nil)
                    }
