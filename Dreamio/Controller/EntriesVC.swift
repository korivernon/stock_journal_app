...
class EntriesVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        searchController.searchBar.tintColor = Colors.purpleDarker
        searchController.searchBar.barTintColor = Colors.purpleDarker
    ...
    func filterContentForSearchText(_ searchText: String, scope: String = "All") {
       ...
       filteredEntries = self.entries.filter({( user : Entry) -> Bool in
           return user.title!.lowercased().contains(searchText.lowercased())
       })
        
       print("filteredEntries \(filteredEntries.count)")
            ...
            deleteAction: { [unowned self] in
                Alerts.showWarningAlertWithCancelAndCustomAction(title: "Delete Entry?", subTitle: "Are you sure you want to delete this entry? We cannot retrieve it once deleted.", actionTitle: "Yes", action: { [unowned self] in
                  ...
                  //  if CheckInternet.isConnectedToNetwork() {
                        ...
                        //remove pic entry
                        self.ref.child("entries").child(id).child("photoUrls").setValue([])
...
extension EntriesVC: UISearchControllerDelegate{
    
}

extension EntriesVC: UISearchBarDelegate{
    
}

extension EntriesVC: UISearchResultsUpdating {
