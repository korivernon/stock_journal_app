...
class NotebookVC: UIViewController,UIGestureRecognizerDelegate {
    ...
    @objc func handleLongPress(gestureRecognizer: UILongPressGestureRecognizer) {
        ...
        if let indexPath = collectionView?.indexPathForItem(at: p) {
            let alertController = UIAlertController(title: "Select action", message: nil, preferredStyle: UIAlertController.Style.actionSheet)
            
            let shareNotebook = UIAlertAction(title: "Share Notebook", style: .default, handler: { (action) -> Void in
                self.shareNotebookCover(with: self.notebooks[indexPath.row].id!)
            })
            let renameNotebook = UIAlertAction(title: "Rename Notebook", style: .default, handler: { (action) -> Void in
                self.renameNotebook(with: self.notebooks[indexPath.row].id!)
            })
            let deleteNotebook = UIAlertAction(title: "Delete Notebook", style: .destructive, handler: { (action) -> Void in
                self.deleteNotebook(with: self.notebooks[indexPath.row].id!)
            })
            let cancel = UIAlertAction(title: "Cancel", style: .cancel, handler: { (action) -> Void in
            })
            alertController.addAction(shareNotebook)
            alertController.addAction(renameNotebook)
            alertController.addAction(deleteNotebook)
            alertController.addAction(cancel)
            //alertController.addAction(removepicture)
            if let popoverPresentationController = alertController.popoverPresentationController {
               popoverPresentationController.sourceView = self.view
                popoverPresentationController.sourceRect = gestureRecognizer.view!.bounds
            }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == Segues.NotebooksToShareNotebookVC{
            let destinationVC = segue.destination as! ShareNotebookVC
...
extension NotebookVC : NotebookCellDelegate {
    ...
    func shareNotebookCover(with id: String) {
        performSegue(withIdentifier: Segues.NotebooksToShareNotebookVC, sender: id)
    }
